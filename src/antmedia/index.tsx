...
const AntMediaContext = createContext<Omit<Values, 'children'>>({
  initialized: false,
...
function AntMediaProvider({ children, ...props }: Values) {
...
export default function withAntmedia<P = any>(
  ...
  class WebRTCAdaptorWrapper extends React.Component<IProps, IStates> {
    ...
    switchCanvasStream(streamId: string) {
      try {
        const {
          configs, settings
        } = this.props;
        const appName = configs.appName || settings.AntMediaAppname;
        if (appName !== 'WebRTCAppEE') return;

        const canvas = document.createElement('canvas');
        const canvasContext = canvas.getContext('2d');

        const video = document.createElement('video');
        const localStream = this.webRTCAdaptor.localStream.clone();
        ...
        this.webRTCAdaptor.updateVideoTrack(canvasStream, streamId);
        ...
        setInterval(() => {
          ...
          // canvasContext.fillText('ABC', 50, 50);
      ...
      return (
        ...
        >
          <WrappedComponent
            ...
            switchCanvasStream={this.switchCanvasStream.bind(this)}
...
export const useVideoDevice = () => {
  const { availableDevices } = useContext(AntMediaContext);

  return availableDevices.filter((device) => device.kind === 'videoinput');
};

export const useWebRTCAdapter = () => {
  const { webRTCAdaptor } = useContext(AntMediaContext);
  return webRTCAdaptor;
};

export const useLocalTrack = () => {
  const { onTrack } = useContext(AntMediaContext);
  return onTrack;
};
