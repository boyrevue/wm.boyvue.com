...
import style from './popup-18plus-content.module.less';
...
interface Ipopup18ContentId {
  settings: any;
}

function Popup18PlusComponent({
  settings
}: Ipopup18ContentId) {
  ...
  const getPopup18PlusContent = async () => {
    setLoading(true);
    const { popup18ContentId } = settings;
    if (popup18ContentId) {
  ...
  useEffect(() => {
    ...
    if (agree18 !== 'yes' && settings.popup18Enabled) {
      getPopup18PlusContent();
    }
  }, [settings]);

  if (!showPopup || loading) return null;

  return (
    <Modal
      width={770}
      centered
      className={style['adult-warning']}
      visible={showPopup}
...
export const Popup18Plus = connect(mapState)(Popup18PlusComponent);
