import {
  getConversationDetail, getConversations, onPinSuccess,
  receiveMessageSuccess, resetConversationState,
  searchConversations, setActiveConversation, updateTotalUnreadMessageInConversation
...
type IProps = {
  ...
  updateTotalUnreadMessageInConversation: Function;
  conversation: {
    list: {
      requesting: boolean;
      error: any;
...
class ConversationList extends PureComponent<IProps> {
  ...
  onMessage = (message: { conversationId: string | number; }) => {
    ...
    const {
      ...
      updateTotalUnreadMessageInConversation: dispatchUpdateeTotalUnreadMessageInConversation
    } = this.props;
    ...
    dispatchUpdateeTotalUnreadMessageInConversation(message);
  };
  ...
  onPinSuccess = (conversationId: string) => {
    const {
      onPinSuccess: handleOnPinSuccess
    } = this.props;
    handleOnPinSuccess({ conversationId });
  };
  ...
  render() {
    ...
    return (
      <div className={style['conversation-list']} onScroll={this.handleScroll.bind(this)}>
        ...
        <h3 className="user-bl" aria-hidden>
          <MessageIcon />
          {' '}
          Conversations
        </h3>
        ...
        <div className={style['c-list-item']}>
          {conversations.length > 0
            && conversations.map((conversationId) => (
              <ConversationListItem
                ...
                onPinSuccess={this.onPinSuccess}
                onUnPinSuccess={this.onUnPinSuccess}
              />
            ))}
          {requesting && (
          ...
          {!requesting && !conversations.length && (
            <p className="conversations-alert-text">No conversations found</p>
          )}
        </div>
      </div>
    );
