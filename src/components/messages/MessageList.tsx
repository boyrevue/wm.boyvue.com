...
import {
  Avatar, Button, message as toasty,
  Spin
} from 'antd';
...
class MessageList extends PureComponent<IProps> {
  ...
  handleSubscribe = async (paymentGateway = 'emerchant', type = 'monthly') => {
          ...
          ? (
            <div className={style['message-list-container']}>
              <div className={style['mess-recipient']}>
                <span
                  ...
                  onClick={() => conversation?.recipientInfo?.isPerformer
                    ...
                    }, `/${conversation?.recipientInfo?.username || conversation?.recipientInfo?._id}`)}
                >
                  ...
                  {' '}
                  {conversation?.recipientInfo?.verifiedAccount && <CheckIcon />}
                </span>
              ...
              {!conversation?.isSubscribed && (
                <SubscriptionPerformerBlock settings={settings} onSelect={this.handleSubscribe} disabled={submiting || (!settings?.ccbillEnabled && !settings?.verotelEnabled && !settings?.emerchantEnabled)} performer={conversation?.recipientInfo} />
              )}
              {conversation.isBlocked && <div className="model-block-message">This model has blocked you!</div>}
            </div>
          )
          : <div className={style['sub-text']}>Select the User to Start Conversations.</div>}
