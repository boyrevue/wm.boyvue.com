...
class PrivateStreamingContainer extends PureComponent<IProps, IState> {
  ...
  async handelWebRTCAdaptorCallback(
    ...
    this.setState({ openAmountPrivateModal: false });
  ...
  leave() {
    if (typeof window !== 'undefined') {
      ...
      message.success('Stream has been stopped. You will be redirected to homepage in 10 seconds');
    }
  }
  ...
  render() {
    ...
    const { loading, openAmountPrivateModal } = this.state;
    ...
    return (
      <>
        ...
        <div className={classNames('show-streaming-button', { 'join-streaming': initialized })}>
          {!initialized ? (
            <>
              {performer && (
              <Button
                type="primary"
                onClick={() => this.setState({ openAmountPrivateModal: true })}
                aria-hidden
                loading={loading}
                disabled={loading}
                block
              >
                <span>Request private live stream</span>
              </Button>
              )}
              {!performer && (
              <Button
                ...
                block
              >
                <span>Accept Private Chat Request</span>
              </Button>
              )}
            </>
          ) : (
            <Button
              className="close-streaming"
              type="primary"
              onClick={this.leave.bind(this)}
              block
              disabled={loading}
            >
              <span className="hide">Stop Streaming</span>
              <CloseOutlined />
            </Button>
          )}
        </div>
        <Modal
          title="Private chat"
          visible={openAmountPrivateModal}
          footer={null}
          destroyOnClose
          centered
          width={300}
          onCancel={() => this.setState({ openAmountPrivateModal: false })}
        >
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <Button
              onClick={() => this.setState({ openAmountPrivateModal: false })}
            >
              <span>Cancel</span>
            </Button>
            <Button
