...
class StreamService extends APIRequest {
  getSessionId(id: string, type: string) {
    return this.get(`/streaming/session/${encodeURIComponent(id)}/${type}`);
  }
  ...
  rejectPrivateList(id: string) {
    return this.del(`/streaming/private/available-list/${id}`);
  }
  ...
  async getPublishToken(options: {
    ...
    try {
      ...
      if (secureOption) {
        ...
        if (!resp.data) throw new Error('Cannot get stream secure token');
